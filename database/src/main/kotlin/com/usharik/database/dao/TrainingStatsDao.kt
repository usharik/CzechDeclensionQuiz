...
interface TrainingStatsDao {
    ...
    @Query("UPDATE daily_training_stats SET score = MAX(0, score + :points), updated_at = :updatedAt WHERE date = :date") suspend fun addScore(date: String, points: Int, updatedAt: Long)
    ...
    @Transaction suspend fun addScorePoints(date: String, points: Int, updatedAt: Long) { insertIgnoreStatsRow(date, updatedAt); addScore(date, points, updatedAt) }
